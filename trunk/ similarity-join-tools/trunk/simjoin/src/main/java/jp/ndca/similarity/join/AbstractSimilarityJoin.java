...
public abstract class AbstractSimilarityJoin implements SimilarityJoin{
	...
	public StringItem strConvert( String data, int id ) {
		return new StringItem( tokenizer.tokenize( data, useConvertingTypeData ), id );
	}
	...
	public StringItem[] strConvert( List<String> dataSet ) {
		...
		for( int i = 0 ; i < len ; i++ )
			_dataSet[i] = new StringItem( tokenizer.tokenize( dataSet.get(i), useConvertingTypeData ), i );
